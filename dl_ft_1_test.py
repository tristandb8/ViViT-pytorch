...
class ek_test(Dataset):
    ...
    def __getitem__(self,index):
      #I need one clip at a time i.e. 10 voxels
        clip_1,clip_2,clip_3,clip_4,clip_5, clip_class   = self.process_data(index)
    ...
    def build_clip(self, vid_path):
       ...
       if(frame_count<(self.num_frames*self.num_clips_test)):
         temp = np.linspace(0,frame_count-1,frame_count,dtype=int)
         frames_dense = np.resize(temp, (self.num_frames*self.num_clips_test))
         frames_dense = np.reshape(frames_dense,(self.num_clips_test, self.num_frames))

       elif(frame_count==(self.num_frames*self.num_clips_test)):
         frames_dense = np.linspace(0,frame_count-1,(self.num_frames*self.num_clips_test),dtype=int)
         frames_dense = np.reshape(frames_dense,(self.num_clips_test, self.num_frames))

       elif(frame_count>(self.num_frames*self.num_clips_test)):
          start = np.random.randint(frame_count-(self.num_frames*self.num_clips_test))
          frames_dense = np.linspace(start,start+(self.num_frames*self.num_clips_test-1),(self.num_frames*self.num_clips_test),dtype=int)
          frames_dense = np.reshape(frames_dense,(self.num_clips_test, self.num_frames))
...
if __name__ == '__main__':
  ...
  for i, (clip_1,clip_2,clip_3,clip_4,clip_5, clip_class) in enumerate(train_dataloader):
    #vis_frames(clip_1[0,:],'clip1','/home/adeel/Documents/testing/')
    #vis_frames(clip_2[0,:],'clip2','/home/adeel/Documents/testing/')
    #vis_frames(clip_3[0,:],'clip3','/home/adeel/Documents/testing/')
    #vis_frames(clip_4[0,:],'clip4','/home/adeel/Documents/testing/')
    #vis_frames(clip_5[0,:],'clip5','/home/adeel/Documents/testing/')
    print(i)
